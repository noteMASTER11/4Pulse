...
const RECONNECT_MAX_MS  = 30_000; // ★ OPT: 30с достаточно, 64с слишком долго
...
export class ForpdaWebSocket {
    ...
    #callbacks = {
        ...
        onBookmarks:  null,
        /** Ответ на запрос истории просмотров: (topics: Array) => void */
        onHistory:    null,
     ...
     * reqId → { resolve, reject, timeoutId, cmdType }
     * @type {Map<number, {resolve: Function, reject: Function, timeoutId: number, cmdType: string}>}
     */
    #pendingCmds = new Map();
    ...
    async connect() {
        ...
        if (!this.#passHash) {
            ...
            if (!hash) {
                debugWarn('[WS] pass_hash не найден — подключение отменено');
                return;
            }
            this.#passHash = hash;
        }
    ...
    async requestBookmarks() {
        if (!this.isConnected) {
            debugWarn('[WS] requestBookmarks: сокет не готов (переподключение?)');
            return;
        }
    ...
    /**
     * Запрашивает историю последних просмотренных тем (команда "mh").
     * Результат придёт в колбэк onHistory.
     * @param {number} offset — смещение (0 = с начала)
     * @param {number} perPage — количество тем (макс. 15)
     */
    async requestHistory(offset = 0, perPage = 15) {
        if (!this.isConnected) {
            debugWarn('[WS] requestHistory: сокет не готов');
            return;
        }
        const reqId = makeReqId();
        try {
            await this.#sendCmd(reqId, [reqId, 'mh', offset, perPage], 'mh');
        } catch (e) {
            console.error('[WS] requestHistory ошибка:', e);
        }
    }

    /**
     * Запрашивает историю несколькими страницами, чтобы счётчик показывал
     * реальное количество доступных тем, а не фиксированные 15 с первой страницы.
     * Серверная команда mh отдаёт страницы по offset/perPage.
     */
    async requestHistoryAll(maxItems = 75, perPage = 15) {
        if (!this.isConnected) {
            debugWarn('[WS] requestHistoryAll: сокет не готов');
            return;
        }

        const all = [];
        const seen = new Set();
        const safePerPage = Math.max(1, Math.min(15, Number(perPage) || 15));
        const safeMax = Math.max(safePerPage, Math.min(150, Number(maxItems) || 75));

        for (let offset = 0; offset < safeMax; offset += safePerPage) {
            const reqId = makeReqId();
            let msg;
            try {
                msg = await this.#sendCmdFull(reqId, [reqId, 'mh', offset, safePerPage], 'mh_all');
            } catch (e) {
                debugWarn('[WS] requestHistoryAll: страница истории не получена:', offset, e);
                break;
            }

            const raw = Array.isArray(msg?.[2]) ? msg[2]
                      : Array.isArray(msg?.[3]) ? msg[3]
                      : [];
            ...
            for (const item of raw) {
                ...
                if (!topicId || seen.has(String(topicId))) continue;
                seen.add(String(topicId));
                all.push(item);
                added++;
            }

            if (raw.length < safePerPage || added === 0) break;
        }

        debugLog(`[WS] История all: ${all.length} тем`);
        this.#processHistory(all);
    }
    ...
    async deleteBookmarkWs(id, bm) {
        ...
        try {
            ...
            debugLog('[WS] deleteBookmarkWs status=', status);
            return status === 0;
        } catch (e) {
    ...
    async addFolderWs(title, parentId = 0, sortOrder = 0) {
        ...
        try {
            ...
            debugLog('[WS] addFolderWs status=', status);
            return status === 0;
        } catch (e) {
    ...
    async syncBookmarksWs(rawEntries, renames = new Map(), deletedIds = new Set()) {
        ...
        const reqId = makeReqId();
        try {
            debugLog(`[WS] syncBookmarksWs: отправляем ${entries.length} записей, renames=${renames.size}, deleted=${deletedIds.size}`);
            ...
            debugLog('[WS] syncBookmarksWs status=', status);
            return status === 0;
        } catch (e) {
    ...
    async renameBookmarkWs(id, newTitle, bm) {
        ...
        try {
            ...
            debugLog('[WS] renameBookmarkWs status=', status);
            return status === 0;
        } catch (e) {
    ...
    #openSocket() {
        ...
        debugLog(`[WS] Подключение (попытка #${this.#reconnectAttempt + 1})…`);
        ...
        // ── onopen ───────────────────────────────────────────────
        socket.onopen = async () => {
            debugLog('[WS] Соединение установлено');
        ...
        socket.onclose = (event) => {
            debugWarn(`[WS] Закрыт: code=${event.code} reason="${event.reason}"`);
            this.#socket = null;
            this.#clearPingTimer();
    ...
    async #subscribeToEvents() {
        ...
        debugLog('[WS] Подписка на события оформлена');
    }
    ...
    #handleMessage(text) {
        ...
        try {
            ...
            debugWarn('[WS] Не JSON:', text);
            return;
        }

        if (!Array.isArray(msg) || msg.length < 2) {
            debugWarn('[WS] Неожиданный формат:', msg);
            return;
        }
        ...
        if (id === PUSH_EVENT_ID) {
            ...
            debugLog('[WS] Неизвестный пакет id=%d:', id, msg);
        }
    }
    ...
    #handlePushEvent(msg) {
        ...
        if (status !== 0) {
            debugWarn('[WS] Push-событие с ненулевым статусом:', status);
            return;
        }
        if (typeof eventString !== 'string' || eventString.length < 2) {
            debugWarn('[WS] Неверный eventString:', eventString);
            return;
        }
        ...
        debugLog(`[WS] 🔔 Событие type="${type}" entityId=${entityId} flag=${flag} msgId=${msgId}`);
    ...
    #handleCommandReply(reqId, msg) {
        ...
        if (!pending) return;
        debugLog('[WS] commandReply reqId=', reqId, 'cmdType=', pending.cmdType ?? '', 'status=', msg[1], 'full=', JSON.stringify(msg).slice(0, 150));
        ...
        if (status === 0) {
            if (pending.cmdType === 'mh') {
                // Ответ на запрос истории: [reqId, 0, [[topicId, title, ts, postId?], ...]]
                // Логируем полный ответ для диагностики формата
                debugLog('[WS] mh raw response:', JSON.stringify(msg).slice(0, 300));
                const historyArr = Array.isArray(msg[2]) ? msg[2]
                                 : Array.isArray(msg[3]) ? msg[3]
                                 : null;
                if (historyArr) this.#processHistory(historyArr);
            } else if (Array.isArray(msg[4])) {
                // Ответ на запрос закладок: [reqId, 0, timestamp, [], [[...bookmarks]]]
    ...
    /**
     * Преобразует сырой массив тем истории в объекты и вызывает onHistory.
     * Предполагаемый формат элемента: [topicId, title, timestamp, lastPostId?, section?]
     * (формат уточняется по логам первого реального запроса)
     *
     * @param {Array[]} raw
     */
    #processHistory(raw) {
        // Формат mh: [topicId, title, lastMsgSnippet, repliesCount, forumId, timestamp, lastUserId, lastUserName, lastPostId]
        // Подтверждён логами: [818728,"brant34 – Entwicklung","для работы",968,39,1759617141,1308000,"BrantX",1777914563]

        const topics = raw
            .filter(item => Array.isArray(item) && typeof item[0] === 'number' && item[0] > 0)
            .map(item => {
                const topicId    = item[0];
                const title      = typeof item[1] === 'string' ? item[1] : '';
                const snippet    = typeof item[2] === 'string' ? item[2] : '';
                const replyCnt   = typeof item[3] === 'number' ? item[3] : 0;
                const forumId    = typeof item[4] === 'number' ? item[4] : 0;
                const timestamp  = typeof item[5] === 'number' ? item[5] : 0;
                const lastUserId = typeof item[6] === 'number' ? item[6] : 0;
                const lastUser   = typeof item[7] === 'string' ? item[7] : '';
                const lastPostId = typeof item[8] === 'number' ? item[8] : 0;

                return {
                    id:           topicId,
                    title:        title,
                    snippet:      snippet,
                    reply_count:  replyCnt,
                    forum_id:     forumId,
                    last_post_ts: timestamp,
                    last_user_id: lastUserId,
                    last_user:    lastUser,
                    last_post_id: lastPostId,
                    // Всегда открываем через getlastpost — перенаправляет на последний видимый пост.
                    // findpost&p=NNN не работает для закрытых/удалённых постов.
                    url:       `https://4pda.to/forum/index.php?showtopic=${topicId}&view=getlastpost`,
                    topic_url: `https://4pda.to/forum/index.php?showtopic=${topicId}`,
                };
            });

        debugLog(`[WS] История: ${topics.length} тем`, topics.slice(0, 2).map(t => `#${t.id} "${t.title}"`));
        this.#callbacks.onHistory?.(topics);
    }
    ...
    #processBookmarks(raw) {
        ...
        debugLog(`[WS] Получено закладок: ${bookmarks.length}`);
        // Логируем первые 3 для диагностики — видно изменились ли данные на сервере
        bookmarks.slice(0, 3).forEach(b => {
            debugLog(`[WS] bk id=${b.id} title="${b.title}" deleted=${b.deleted}`);
        });
     ...
     */
    #sendCmd(reqId, payload, cmdType = '') {
        return new Promise((resolve, reject) => {
            const timeoutId = setTimeout(() => {
                this.#pendingCmds.delete(reqId);
                reject(new Error(`[WS] Команда reqId=${reqId} (${cmdType}) не получила ответа (таймаут)`));
            }, CMD_TIMEOUT_MS);

            this.#pendingCmds.set(reqId, { resolve, reject, timeoutId, cmdType });
            this.#rawSend(payload);
        });
    }

    /** Как #sendCmd, но возвращает полный ответ сервера. Используется для сборки истории по страницам. */
    #sendCmdFull(reqId, payload, cmdType = '') {
        return new Promise((resolve, reject) => {
            const timeoutId = setTimeout(() => {
                this.#pendingCmds.delete(reqId);
                reject(new Error(`[WS] Команда reqId=${reqId} (${cmdType}) не получила ответа (таймаут)`));
            ...
            this.#pendingCmds.set(reqId, { resolve, reject, timeoutId, cmdType, fullResponse: true });
            this.#rawSend(payload);
        });
    }
    ...
    #rawSend(payload) {
        if (this.#socket?.readyState !== WebSocket.OPEN) {
            debugWarn('[WS] rawSend: сокет не открыт, пакет отброшен:', payload);
            return;
        }
        debugLog('[WS] rawSend:', JSON.stringify(payload).slice(0, 120));
    ...
    #rawSendWin1251(payload) {
        if (this.#socket?.readyState !== WebSocket.OPEN) {
            debugWarn('[WS] rawSendWin1251: сокет не открыт, пакет отброшен');
            return;
        }
        ...
        debugLog('[WS] rawSendWin1251:', json.slice(0, 120));
     ...
     * Задержка: min(BASE * 2^attempt, MAX) + jitter (±15%)
     *   попытка 1 → ~2 с, 2 → ~4 с, 3 → ~8 с, …, 5+ → ~30 с
     */
    #scheduleReconnect() {
        const base = Math.min(
        ...
        // ★ OPT: jitter ±15% — предотвращает thundering herd при массовом реконнекте
        const jitter = base * 0.15 * (Math.random() * 2 - 1);
        const delay = Math.max(500, Math.round(base + jitter));
        this.#reconnectAttempt++;

        debugLog(`[WS] Переподключение через ${delay / 1000} с (попытка #${this.#reconnectAttempt})…`);
...
export async function createWsClient(userId, callbacks = {}) {
    ...
    if (!passHash) {
        debugWarn('[WS] createWsClient: pass_hash не найден, пропускаем');
        return null;
    }
...
export function registerWsKeepAlive() {
    ...
    chrome.alarms.onAlarm.addListener((alarm) => {
        if (alarm.name === WS_KEEPALIVE_ALARM) {
            ...
            debugLog('[WS] Keep-alive alarm');
        }
    });

    debugLog('[WS] Keep-alive alarm зарегистрирован');
}
