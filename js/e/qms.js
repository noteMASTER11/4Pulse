...
export class QMS extends AbstractEntity {
    ...
    async update(forceRefresh = false) {
        // 🔔 FIX: Всегда пробуем HTTP-запрос для получения уведомлений,
        // даже когда WS подключён. WS лишь сигнализирует о событии,
        // а Inspector API возвращает актуальные данные для уведомления.
        // Если Cloudflare заблокировал HTTP — ловим тихо, не ломаем поток.
        try {
            await super.update();
            await this.#fetchFullList(forceRefresh);
        } catch (e) {
            if (this.cs.wsConnected) {
                console.debug('[QMS] HTTP недоступен (Cloudflare?), WS активен — пропуск уведомления');
            } else {
                throw e;
            }
        }
