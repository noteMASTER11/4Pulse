...
export class Mentions extends AbstractEntity {
    ...
    async update(forceRefresh = false) {
        ...
        // 🔔 FIX: Всегда пробуем HTTP-запрос для уведомлений, даже при активном WS.
        // Если Cloudflare блокирует HTTP — ловим ошибку тихо, не ломаем поток.
        try {
            await this.#updateFromInspectorAPI();
        } catch (e) {
            if (this.cs.wsConnected) {
                console.debug('[Mentions] HTTP недоступен (Cloudflare?), WS активен — пропуск уведомления');
            } else {
                throw e;
            }
        }
